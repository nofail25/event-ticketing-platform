<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-xl font-bold text-slate-900">Edit Kategori Tiket</h2>
                <p class="mt-1 text-sm text-slate-500">Perbarui "{{ $ticketCategory->name }}" untuk "{{ $event->title }}".</p>
            </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <form method="POST" action="{{ route('organizer.events.ticket-categories.update', [$event, $ticketCategory]) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $ticketCategory->name)" placeholder="contoh: VIP, Tiket Masuk Umum" required autofocus />
