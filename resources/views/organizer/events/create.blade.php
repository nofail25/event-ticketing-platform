<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-xl font-bold text-slate-900">Buat Event</h2>
                <p class="mt-1 text-sm text-slate-500">Siapkan daftar event baru untuk audiens Anda.</p>
            </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <form method="POST" action="{{ route('organizer.events.store') }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title')" placeholder="contoh: Konferensi Teknologi 2026" required autofocus />
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" class="block mt-1 w-full border-slate-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-lg shadow-sm" placeholder="Jelaskan apa yang dapat diharapkan oleh peserta...">{{ old('description') }}</textarea>
                ...
                <div>
                    ...
                    <x-text-input id="location" class="block mt-1 w-full" type="text" name="location" :value="old('location')" placeholder="contoh: Jakarta Convention Center" required />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="rounded-lg border border-amber-200 bg-amber-50 p-4">
                        <p class="text-sm font-semibold text-amber-900">Persetujuan Diperlukan</p>
                        <p class="mt-1 text-sm text-amber-700">Event baru diserahkan sebagai menunggu keputusan sampai Admin menyetujuinya.</p>
                    </div>

                    <!-- Banner Image -->
                    <div>
                        <x-input-label for="banner_image" :value="__('Gambar Spanduk (Opsional)')" />
