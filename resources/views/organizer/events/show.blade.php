<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Manage event details, tickets, and settings.</p>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            @else
                <div class="w-full aspect-video bg-slate-100 flex items-center justify-center text-slate-400">No Banner Image</div>
            @endif
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-2">
                        <h3 class="text-lg font-bold text-slate-900 mb-2">Description</h3>
                        <p class="text-slate-700 whitespace-pre-line">{{ $event->description ?: 'No description provided.' }}</p>
                    </div>
                    <div>
                        <div class="bg-slate-50 p-4 rounded-lg">
                            <h3 class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Details</h3>
                            <div class="space-y-3">
                                <div>
                                ...
                                <div>
                                    <p class="text-xs text-slate-500">End Time</p>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-6 text-slate-900 border-b border-slate-100 flex justify-between items-center">
                <div>
                    <h3 class="text-lg font-bold text-slate-900">Ticket Categories</h3>
                    <p class="text-sm text-slate-500 mt-1">Manage pricing and availability for each ticket type.</p>
                </div>
                        ...
                        @foreach($event->ticketCategories as $category)
                            <div class="border border-slate-200 rounded-lg p-4 shadow-sm hover:shadow-md transition">
                                ...
                                <div class="flex justify-between text-sm">
                                    <span class="text-slate-500">Price: <span class="font-medium text-slate-900">${{ number_format($category->price, 2) }}</span></span>
                                    <span class="text-slate-500">Quota: <span class="font-medium text-slate-900">{{ $category->quota }}</span></span>
                                </div>
                            </div>
                        @endforeach
                    </div>
                @else
                    <p class="text-slate-500 text-center py-4">No ticket categories added yet.</p>
                @endif
            </div>
        </div>
    </div>
