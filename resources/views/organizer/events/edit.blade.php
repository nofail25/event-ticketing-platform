                    ...
                    <form method="POST" action="{{ route('organizer.events.update', $event) }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
                                <p class="text-sm font-semibold text-yellow-900">Current Status: <span class="capitalize">{{ $event->status }}</span></p>
                                <p class="mt-1 text-sm text-yellow-700">Editing an active event sends it back to pending for Super Admin review.</p>
                            </div>
                            ...
                            <div>
                                ...
                                @if($event->banner_image)
                                    <div class="mt-2 mb-2">
                                        ...
                                        <img src="{{ Storage::url($event->banner_image) }}" alt="Current Banner" class="h-20 object-cover rounded">
                                    </div>
                                @endif
                                ...
                                <p class="mt-1 text-xs text-gray-500">Max 2MB. JPG, PNG, WEBP. Leave empty to keep current.</p>
                            </div>
                        </div>
