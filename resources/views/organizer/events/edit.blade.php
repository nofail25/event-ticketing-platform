    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Update details for "{{ $event->title }}".</p>
            </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <form method="POST" action="{{ route('organizer.events.update', $event) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $event->title)" placeholder="e.g., Tech Conference 2026" required autofocus />
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" class="block mt-1 w-full border-slate-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-lg shadow-sm" placeholder="Describe what attendees can expect...">{{ old('description', $event->description) }}</textarea>
                ...
                <div>
                    ...
                    <x-text-input id="location" class="block mt-1 w-full" type="text" name="location" :value="old('location', $event->location)" placeholder="e.g., Jakarta Convention Center" required />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="rounded-lg border border-amber-200 bg-amber-50 p-4">
                        <p class="text-sm font-semibold text-amber-900">Current Status: <span class="capitalize">{{ $event->status }}</span></p>
                        <p class="mt-1 text-sm text-amber-700">Editing an active event sends it back to pending for admin review.</p>
                    </div>

                    <!-- Banner Image -->
                    <div>
                        <x-input-label for="banner_image" :value="__('Update Banner Image')" />
                        @if($event->banner_image)
                            <div class="mt-2 mb-2">
                                <p class="text-sm text-slate-500 mb-1">Current Banner:</p>
                                <img src="{{ Storage::url($event->banner_image) }}" alt="Current Banner" class="h-20 aspect-video object-cover rounded-lg">
                            </div>
                        @endif
                        ...
                        <p class="mt-1 text-xs text-slate-500">Max 2MB. JPG, PNG, WEBP. Leave empty to keep current. <br>Rekomendasi ukuran: 1920x1080 (Rasio 16:9) agar gambar tampil penuh.</p>
                    </div>
                </div>
